    ...
    while(true){
        if (vel > 0) {
            ...
            case 1: // Trem 1 (Verde)
                ...
                // NOVAS SAÍDAS
                if (x == 40 && y == 50 && insideRegion[5])   { sem_post(&semaforos[5]); insideRegion[5] = false; } // Solta RC5 (Esquerda)
                if (x == 290 && y == 40 && insideRegion[6])  { sem_post(&semaforos[6]); insideRegion[6] = false; } // Solta RC6 (Topo)


                // --- Lógicas de Parada/Movimento (Hierarquia: 0, 2, 5, 6) ---

                // Ponto de Parada RC5 (Trilho Esquerda)
                // Ponto: (40, 150) -> Pede RC5 para subir
                if (x == 40 && y == 150 && !insideRegion[5]) {
                    sem_wait(&semaforos[5]);
                    insideRegion[5] = true;
                    y -= 10; // Sobe
                }
                // Ponto de Parada RC6 (Trilho Topo)
                // Ponto: (50, 40) -> Pede RC6 para ir p/ direita (JÁ POSSUI RC5)
                else if (x == 50 && y == 40 && !insideRegion[6]) {
                    sem_wait(&semaforos[6]);
                    insideRegion[6] = true;
                    x += 10; // Direita
                }
                // Ponto de Parada RC 0 (Interno)
                // Ponto: (290, 40) -> Pede RC0 (JÁ POSSUI RC5 e RC6)
                else if (x == 290 && y == 40 && !insideRegion[0]) {
                    sem_wait(&semaforos[0]);
                    insideRegion[0] = true;
                    x += 10;
                }
                // Ponto de Parada RC 2 (Interno)
                // Ponto: (310, 140) -> Pede RC2 (JÁ POSSUI RC0)
                ...
                // Movimento Normal
                    ...
                    if (y == 40 && x < 310) // (Dentro RC5 e RC6)
                        x += 10;
                    else if (x == 310 && y < 160) // (Dentro RC0)
                        y += 10;
            ...
            case 5: //Trem 5 (Roxo)
                ...
                // ---------- CASE 6 (Trem 6) ----------
            case 6: // Trem 6 (Preto) - SENTIDO HORÁRIO (Seguro)
                /*
                // Posição inicial: (40, 280)

                // --- Lógicas de Saída (Post) ---
                if (x == 40 && y == 40 && insideRegion[5])  { sem_post(&semaforos[5]); insideRegion[5] = false; } // 1. Solta Esquerda
                if (x == 850 && y == 40 && insideRegion[6])  { sem_post(&semaforos[6]); insideRegion[6] = false; } // 2. Solta Topo
                if (x == 850 && y == 280 && insideRegion[7]) { sem_post(&semaforos[7]); insideRegion[7] = false; } // 3. Solta Direita
                if (x == 40 && y == 280 && insideRegion[8])  { sem_post(&semaforos[8]); insideRegion[8] = false; } // 4. Solta Base

                // --- Lógicas de Entrada (Wait) ---

                // 1. Ponto de Parada RC5 (Trilho Esquerda)
                // Ponto: (40, 270) -> Pede RC5 para subir
                if (x == 40 && y == 270 && !insideRegion[5]) {
                    sem_wait(&semaforos[5]);
                    insideRegion[5] = true;
                    y -= 10; // Sobe
                }
                // 2. Ponto de Parada RC6 (Trilho Topo)
                // Ponto: (50, 40) -> Pede RC6 para ir p/ direita
                else if (x == 50 && y == 40 && !insideRegion[6]) {
                    sem_wait(&semaforos[6]);
                    insideRegion[6] = true;
                    x += 10; // Direita
                }
                // 3. Ponto de Parada RC7 (Trilho Direita)
                // Ponto: (850, 50) -> Pede RC7 para descer
                else if (x == 850 && y == 50 && !insideRegion[7]) {
                    sem_wait(&semaforos[7]);
                    insideRegion[7] = true;
                    y += 10; // Desce
                }
                // 4. Ponto de Parada RC8 (Trilho Base)
                // Ponto: (840, 280) -> Pede RC8 para ir p/ esquerda
                else if (x == 840 && y == 280 && !insideRegion[8]) {
                ...
                // Movimento Normal
                    ...
                    if (x == 40 && y > 40) // 1. (Dentro RC5) Sobe pela Esquerda
                        y -= 10;
                    else if (y == 40 && x < 850) // 2. (Dentro RC6) Direita no Topo
                        x += 10;
                    else if (x == 850 && y < 280) // 3. (Dentro RC7) Desce pela Direita
                        y += 10;
                    else if (y == 280 && x > 40) // 4. (Dentro RC8) Esquerda na Base
                        x -= 10;
                }
                */
                break;
